<div>
    <div class="sm:py-16">
        <div class="px-6 m-auto text-gray-500 xl:container md:px-12">
            <div class="m-auto space-y-8 sm:w-4/5 md:w-3/5 lg:w-2/5">
                <div class="p-8 md:py-12">
                    <a href="#">
                        <svg class="h-10 text-primary dark:text-primaryLight" viewBox="0 0 942 272" fill="none"
                    ...
                    <form action="" class="space-y-8">
                        <div class="space-y-4">
                            <label for="username" class="text-gray-600 dark:text-gray-300">What's your phone number or
                                email
                                ?</label>
                            <div class="relative flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor"
                                    class="absolute inset-y-0 w-6 h-6 my-auto left-4 dark:text-gray-400">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                                </svg>
                                <input type="text" wire:model.defer="emailOrnumber" name="username" id="username"
                                    autocomplete="username" placeholder="Phone number or email"
                                    class="block w-full h-12 pl-12 pr-4 text-gray-600 placeholder-gray-500 transition duration-300 bg-gray-100 rounded-full focus:outline-none dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-300 invalid:ring-2 invalid:ring-red-400 focus:ring-2 focus:ring-primary"
                                    @if($isTrue) disabled @endif />
                                <div class="absolute right-1">
                                    @if(!$isTrue)
                                    <button type="button" wire:click="verifiyMail()"
                                        class="relative flex items-center justify-center w-10 h-10 ml-auto sm:w-max sm:px-6 before:absolute before:inset-0 before:rounded-full before:bg-primary dark:before:bg-primaryLight before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95">
                                        <span
                                            class="relative hidden text-base font-semibold text-white dark:text-gray-900 sm:block">Next</span>
                                ...
                                <div wire:loading wire:target='verifiyMail'>
                        ...
                        <div class="space-y-4">
                            ...
                            <x-inputs.password wire:model.defer="password" placeholder="Password"
                                class="block w-full h-12 text-gray-600 placeholder-gray-500 transition duration-300 bg-gray-100 !rounded-full focus:outline-none dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-300 invalid:ring-2 invalid:ring-red-400 focus:ring-2 focus:ring-primary" />
                        ...
                        <div class="flex items-center justify-center w-full px-6">
                            <x-jet-button type="button" wire:click="loginUser()"
                                class="flex items-center justify-center w-full h-10 transition duration-300 rounded-full hover:bg-primary bg-primaryLight dark:bg-primaryLight hover:scale-105 active:duration-75 active:scale-95">

                                <span wire:loading.remove wire:target='loginUser'>{{ __('Log in') }}</span>
                                <span wire:loading wire:target='loginUser'>{{ __('Connexion....') }}</span>

                            </x-jet-button>
                        </div>

                        @endif
                        <p class="text-sm text-gray-500">By proceeding, you consent to get calls, WhatsApp or SMS
                            messages,
                            including by automated means, from Tailus and its affiliates to the number provided.</p>

                        <p
                            class="pt-6 text-sm text-gray-500 border-t border-gray-100 dark:border-gray-700 dark:text-gray-400">
